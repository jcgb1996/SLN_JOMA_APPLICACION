...
namespace COM.JOMA.EMP.QUERY.SERVICE.Model
{
    ...
    {
        internal async Task<List<TerapistasGridQueryDto>> GetTerapistasXRucEmpresa(string RucEmpresa)
        {
            var SP_NAME = "[dbo].[QRY_GetTerapistasXRucEmpresa]";
            List<TerapistasGridQueryDto>? Result = new();
            switch (QueryParameters.TipoORM)
            {
                ...
                case JOMATipoORM.Dapper:
                    using (var connection = jomaQueryContextDP.CreateConnection())
                    {
                        ...
                        Result = (await connection.QueryAsync<TerapistasGridQueryDto>(SP_NAME, parameters, commandType: CommandType.StoredProcedure)).ToList();
                    }
                    break;
            ...
            return Result != null ? Result : new List<TerapistasGridQueryDto>();
        }
    }
}
