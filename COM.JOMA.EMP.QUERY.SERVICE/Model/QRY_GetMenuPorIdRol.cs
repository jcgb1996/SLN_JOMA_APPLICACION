...
namespace COM.JOMA.EMP.QUERY.SERVICE.Model
{
    ...
    {
        internal async Task<List<MenuQueryDto>> GetMenuPorIdRol(long IdUsuario)
        {
            var SP_NAME = "[QRY_GetMenuPorIdRol]";
            ...
            switch (QueryParameters.TipoORM)
            {
                ...
                case JOMATipoORM.Dapper:
                    using (var connection = jomaQueryContextDP.CreateConnection())
                    {
                        ...
                        parameters.Add("@IdRol", JOMAConversions.NothingToDBNULL(IdUsuario), DbType.Int64);
