...
namespace COM.JOMA.EMP.QUERY.SERVICE.Model
{
    ...
    {
        internal async Task<List<TipoTerapiaQueryDto>> GetTipoTerapiasXIdEmpresa(long IdEmpresa)
        {
            var SP_NAME = "[dbo].[QRY_GetTipoTerapiaXIdEmpresa]";
            List<TipoTerapiaQueryDto>? Result = new();
            switch (QueryParameters.TipoORM)
            {
                case JOMATipoORM.EntityFramework:
                    Result = lstTipoTerapiaQueryDto?.FromSqlRaw($"[{SP_NAME}] @p0",
                ...
                case JOMATipoORM.Dapper:
                    using (var connection = jomaQueryContextDP.CreateConnection())
                    {
                        ...
                        Result = (await connection.QueryAsync<TipoTerapiaQueryDto>(SP_NAME, parameters, commandType: CommandType.StoredProcedure)).ToList();
                    }
                    break;
            ...
            return Result != null ? Result : new List<TipoTerapiaQueryDto>();
        }
    }
}
