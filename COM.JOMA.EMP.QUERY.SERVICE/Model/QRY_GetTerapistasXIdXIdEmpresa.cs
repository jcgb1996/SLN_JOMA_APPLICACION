...
namespace COM.JOMA.EMP.QUERY.SERVICE.Model
{
    ...
    {
        internal async Task<TerapistaQueryDto?> GetTerapistasXIdXIdEmpresa(long IdTerapista, long IdEmpresa)
        {
            var SP_NAME = "[dbo].[QRY_GetTerapistasXIdXIdEmpresa]";
            ...
            switch (QueryParameters.TipoORM)
            {
                case JOMATipoORM.EntityFramework:
                    Result = terapistaQueryDto.FromSqlRaw($"[{SP_NAME}] @p0,@p1",
                        JOMAConversions.NothingToDBNULL(IdTerapista),
                ...
                case JOMATipoORM.Dapper:
                    using (var connection = jomaQueryContextDP.CreateConnection())
                    {
                        ...
                        parameters.Add("@IdTerapista", JOMAConversions.NothingToDBNULL(IdTerapista), DbType.Int64);
